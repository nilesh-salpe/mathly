...
- `index.html` — home page with the menu (Multiplication Tables, Quiz Time).
...
- `quiz.html` — timed quiz (**Quiz Time** on the menu). Tick the kinds of sums you want — adding, taking away, times, sharing (division), percentages, fractions — then choose a number range, how many numbers per sum (2 = a×b, 3 = a×b×c, 4 = a×b×c×d, or a mix; applies to + − × ÷), how many sums, and a timer (minutes + seconds; 0:00 means no clock). Subtraction never goes below zero, division always divides exactly, and percentages/fractions always land on whole answers. Each answer is marked individually, you get a score out of the total with stars, and every quiz is kept in a **My scores** list (saved in the browser). **New quiz** deals a fresh set with the same settings.
